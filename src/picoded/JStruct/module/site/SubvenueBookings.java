...
public class SubvenueBookings {
	...
	public GenericConvertList<MetaObject> getSubvenueBookings_bySubVenueId(String in_subvenueId) {
		...
		// Fetch and populate
		MetaObject[] queryRet = subvenueBookings.query("_subvenueID=? AND (_status=? OR _status=?)", new String[] { in_subvenueId , "Paid" , "Approved" }, "_createdTime", 0, bookings_max);
	...
	public GenericConvertList<MetaObject> getSubvenueBookings_byBookingId(String in_bookingID) {
		...
		// Fetch and populate
		MetaObject[] queryRet = subvenueBookings.query("_oid=?", new String[] { in_bookingID }, "_createdTime", 0, bookings_max);
		if (queryRet != null && queryRet.length > 0) {
			for (int i = 0; i < queryRet.length; ++i) {
				ret.add(queryRet[i]);
			}
	...
	public GenericConvertList<MetaObject> getSubvenueBookingDates_byBookingId(String in_subvenueId) {
		...
		// Fetch and populate
		MetaObject[] queryRet = subvenueBookingDates.query("_bookingID=?", new String[] { in_subvenueId }, "_createdTime", 0, bookings_max);
		if (queryRet != null && queryRet.length > 0) {
			for (int i = 0; i < queryRet.length; ++i) {
				ret.add(queryRet[i]);
			}
	...
	///
	public GenericConvertMap<String, Object> createSubvenueBooking(String subvenueID, String eventID , String venueID , Float paymentAmount) {
	...
	/// @return List of meta objects representing the owner
	///
	public GenericConvertList<MetaObject> updateSubvenueBookingStatus(String in_subvenueId , String newStatus) {
	...
	public GenericConvertMap<String, Object> createBookingSlots(String bookingID ,  Map<String, Object> datesList) throws Exception{
		// Prepare the actual return object
		GenericConvertHashMap<String, Object> resMap = new GenericConvertHashMap<String, Object>();
		...
		for(String dateKey : datesList.keySet()){
			...
			if (timeslot_list_raw instanceof List){
				...
				throw new Exception("timeslot_list_raw was not a List<Object>, was a " + timeslot_list_raw.getClass());
			}
		}
