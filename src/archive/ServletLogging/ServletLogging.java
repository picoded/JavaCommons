...
public class ServletLogging {
	...
	private void tableSetup(JSql jSql) throws JSqlException {
		...
		List<String> columnDefine = new ArrayList<String>(Arrays.asList((keyColumnType + pKeyColumnType), keyColumnType,
			tStampColumnType, keyColumnType, keyColumnType, keyColumnType, bitColumnType, keyColumnType));
		...
		jSql.createTableQuerySet("exception", columnName.toArray(new String[columnName.size()]),
			columnDefine.toArray(new String[columnDefine.size()])).execute();
	}
	...
	public void log(JSql jSql, String format, Object... args) throws Exception {
			...
			if (lastIndex != -1) {
				if (format.charAt(lastIndex + 1) == 's') {
					if (stringIndexedCount > stringIndexed) {
						...
						sql += ", " + stringIndexedPrefix + (stringIndexedCount < 10 ? "0" : "") + stringIndexedCount++;
						sqlValues += ", ?";
						values.add(args[index++]);
					}
				} else if (format.charAt(lastIndex + 1) == 'i' || format.charAt(lastIndex + 1) == 'd'
					|| format.charAt(lastIndex + 1) == 'f') {
					if (longIndexedCount > longIndexed) {
						extraLong.add(new Long(String.valueOf(args[index++])));
					} else {
						sql += ", " + longIndexedPrefix + (longIndexedCount < 10 ? "0" : "") + longIndexedCount++;
						sqlValues += ", ?";
						values.add(args[index++]);
					}
				}
		...
		// insert to `logTable`
		jSql.execute(sql, values.toArray(new Object[values.size()]));
						...
						if (format.charAt(lastIndex + 1) == 's') {
							if (stringIndexedCount > stringIndexed) {
								...
								if (val != null && val.toString().trim().length() != 0
									&& !"null".equalsIgnoreCase(val.toString().trim())) {
		...
		jSql
			.execute(
				"INSERT INTO `exception` "
					...
					+ " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", systemHash(jSql), requestID(jSql), createTime(),
				fmtHash, args);
