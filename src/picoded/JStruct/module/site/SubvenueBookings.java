...
public class SubvenueBookings {
	...
	public GenericConvertList<MetaObject> getSubvenueBookings_bySubVenueId(String in_subvenueId) {
		...
		// Fetch and populate
		MetaObject[] queryRet = subvenueBooking.query("_subvenueID=? AND (_status=? OR _status=?)", new String[] { in_subvenueId , "Paid" , "Approved" }, createdTime, 0, bookingsMax);
	...
	public GenericConvertList<MetaObject> getSubvenueBookings_byBookingId(String in_bookingID) {
		...
		// Fetch and populate
		MetaObject[] queryRet = subvenueBooking.query("_oid=?", new String[] { in_bookingID }, createdTime, 0, bookingsMax);
		if (queryRet != null && queryRet.length > 0) {
			for (int i = 0; i < queryRet.length; ++i) {
				ret.add(queryRet[i]);
			}
	...
	public GenericConvertList<MetaObject> getSubvenueBookingDates_byBookingId(String in_subvenueId) {
		...
		// Fetch and populate
		MetaObject[] queryRet = subvenueBookingDates.query("_bookingID=?", new String[] { in_subvenueId }, createdTime, 0, bookingsMax);
		if (queryRet != null && queryRet.length > 0) {
			for (int i = 0; i < queryRet.length; ++i) {
				ret.add(queryRet[i]);
			}
