...
public class BaseX {
	...
	public BaseX(String customCharset) {
		if (customCharset == null || customCharset.length() <= 1) {
			throw new IllegalArgumentException(
					"Charset needs atleast 2 characters");
		}

		inCharset = customCharset;
		inCharsetLength = BigInteger.valueOf(customCharset.length());

		bitToStringCache = sharedBitToStringLengthCache.get(customCharset
				.length());
		stringToBitCache = sharedStringToBitLengthCache.get(customCharset
				.length());

		if (bitToStringCache == null) {
			bitToStringCache = new HashMap<Integer, Integer>();
			sharedBitToStringLengthCache.put(inCharset.length(),
					bitToStringCache);

			stringToBitCache = new HashMap<Integer, Integer>();
			sharedStringToBitLengthCache.put(inCharset.length(),
					stringToBitCache);
		}
	}
	...
	public String encode(byte[] bArr) {
		...
		BigInteger[] dSplit = new BigInteger[] { (new BigInteger(1, bArr)),
				null };
			...
			if (indx < 0) {
				throw new IllegalArgumentException(
						"Invalid character(s) in string: `" + character
								+ "` for full string:" + encodedString);
			}

			encodedValue = encodedValue.multiply(inCharsetLength).add(
					BigInteger.valueOf(indx));
		}
		...
		if (fullEncodedValue.length > byteLength) {
			...
			for (int a = 0; a < copyFrom; ++a) {
				if (fullEncodedValue[a] != 0) {
					logger.warning("Encoded value loss for givent byteLength("
							+ byteLength + ") for input encodedString: "
							+ encodedString);
				}
			}
