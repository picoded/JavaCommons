...
public class AccountLogin extends BasePage {
	...
	public static Map<String, Object> prepareAuthenticatedREST(RESTRequest req,
		...
		// Run actual logic
		// ----------------------------------------
		return call.apply(req, res, basePageObj, accountTableObj, currentUser,
				null, null);
	}
	...
	public static Map<String, Object> fetchGroupObject_fromFirstWildcard_orCurrentUser(
			RESTRequest req, Map<String, Object> res,
			boolean callOnlyForGroupAdmin, loginREST call) {

		// Only runs function if logged in
		return prepareAuthenticatedREST(req, res, (reqObj, resMap, basePageObj,
				accountTableObj, currentUser, accObj_a, accObj_b) -> {
			...
			// ----------------------------------------
				...
				resMap.put("groupID_names",
						new ArrayList<String>(groupObj.getNames()));
				boolean hasGroupadmin = false;

				// Check the group admin role
				if (currentUser.isSuperUser()
						|| groupObj.getMemberRole(currentUser)
				...
				// group object
				return call.apply(reqObj, resMap, basePageObj, accountTableObj,
						currentUser, groupObj, null);
			});
			...
			if (ao != null) {
				...
				if (sanitiseOutput && names != null) {
					for (int i = 0; i < names.length; ++i) {
						names[i] = RegexUtils
								.sanitiseCommonEscapeCharactersIntoAscii(names[i]);
					}
				}

				res.put("accountNames", Arrays
						.asList((names == null) ? new String[] {} : names));
	...
	public static RESTFunction login_POST = (req, res) -> {
		...
		if (req.requestPage() != null) {
			...
			if (ao != null) {
				...
				if (timeAllowed == 0) {
					ao = at.loginAccount(bp.getHttpServletRequest(),
							bp.getHttpServletResponse(), ao,
					...
					if (ao != null) {
						...
						boolean sanitiseOutput = req.getBoolean(
								"sanitiseOutput", true);
						String[] names = ao.getNames().toArray(new String[0]);
						if (sanitiseOutput && names != null) {
							for (int i = 0; i < names.length; ++i) {
					...
					res.put("isLogin", false);
					res.put("error", "Unable to login, user locked out for "
							+ timeAllowed + " seconds.");
				}
	...
	public static RESTFunction logout_GET = (req, res) -> {
		...
		if (req.requestPage() != null) {
			...
			at.logoutAccount(bp.getHttpServletRequest(),
					bp.getHttpServletResponse());
			res.put("logout", "true");
		} else {
			res.put("error", MISSING_REQUEST_PAGE);
		}
		...
		if (req.requestPage() != null) {
			...
			if (ao != null) {
				...
				if (ao._oid().equals(accID)) {
					if (ao.setPassword(req.getString("newPassword"),
							req.getString("oldPassword"))) {
					...
					if (subUser != null) {
						res.put("accountID", accID);
						res.put("success", (subUser.setPassword(req
								.getString("newPassword")) == true));
					} else {
					...
					res.put("error",
							"User does not have permission to edit password: "
									+ accID);
				}
			}
		} else {
	...
	public static RESTFunction infoByName_GET = (req, res) -> {
		if (req.requestPage() != null) {
			...
			boolean sanitiseOutput = req.getBoolean("sanitiseOutput", true);
			Map<String, Object> commonInfo = extractCommonInfoFromAccountObject(
					account, sanitiseOutput);
	...
	private static Map<String, Object> extractCommonInfoFromAccountObject(
			AccountObject account, boolean sanitiseOutput) {
			...
			if (accNameSet != null) {
				...
				if (sanitiseOutput && accNames != null) {
					for (int i = 0; i < accNames.length; ++i) {
						accNames[i] = RegexUtils
								.sanitiseCommonEscapeCharactersIntoAscii(accNames[i]);
					}
				}
				...
				for (AccountObject group : groups) {
					...
					if (sanitiseOutput) {
						...
						for (String groupName : group.getNames()) {
							groupNames
									.add(RegexUtils
											.sanitiseCommonEscapeCharactersIntoAscii(groupName));
						}
					}
					...
					if (sanitiseOutput) {
						role = RegexUtils
								.sanitiseCommonEscapeCharactersIntoAscii(role);
					}
	...
	public static RESTFunction list_GET_and_POST = (req, res) -> {
		return prepareAuthenticatedREST(req,
				res,
				(reqObj, resMap, basePageObj, accountTableObj, currentUser,
						groupObj, accObj_b) -> {
					// cannot fall through
				...
				String[] hasGroupRole_any = req
						.getStringArray("hasGroupRole_any");
				...
				if (query != null && !query.isEmpty() && queryArgs != null
						&& queryArgs.length > 0) {
					res.put("recordsFiltered",
							mtObj.queryCount(query, queryArgs));
				}
				...
				try {
					data = list_GET_and_POST_inner(accountTableObj, draw,
							start, limit, headers, query, queryArgs,
							orderByStr, insideGroupAny, hasGroupRole_any,
							groupStatus, sanitiseOutput);
					res.put("data", data);
				} catch (Exception e) {
	...
	private static List<List<Object>> list_GET_and_POST_inner(
			AccountTable _metaTableObj, int draw, int start, int length,
			String[] headers, String query, String[] queryArgs, String orderBy,
			String[] insideGroup_any, String[] hasGroupRole_any,
			String groupStatus, boolean sanitiseOutput) throws RuntimeException {
		...
		try {
			if (headers != null && headers.length > 0) {
				...
				if ((insideGroup_any == null || insideGroup_any.length == 0)
					...
					// do normal query
					MetaTable accountMetaTable = _metaTableObj
							.accountMetaTable();
					...
					if (query == null || query.isEmpty() || queryArgs == null
							|| queryArgs.length == 0) {
						metaObjs = accountMetaTable.query(null, null, orderBy,
								start, length);
					} else {
						metaObjs = accountMetaTable.query(query, queryArgs,
								orderBy, start, length);
					}

					List<AccountObject> retUsers = new ArrayList<AccountObject>();
					for (MetaObject metaObj : metaObjs) {
						AccountObject ao = _metaTableObj.getFromID(metaObj
								._oid()); // a single account
						retUsers.add(ao);
					}

					fullUserArray = retUsers.toArray(new AccountObject[retUsers
							.size()]);
				} else {
					// do filtered query
					fullUserArray = _metaTableObj.getUsersByGroupAndRole(
							insideGroup_any, hasGroupRole_any);
				}
				...
				if (groupStatus != null) {
					...
					fullUserArray = filteredUsers
							.toArray(new AccountObject[filteredUsers.size()]);
				}
					...
					for (String header : headers) {
						if (header.equalsIgnoreCase("names")) {
							if (ao != null) {
								...
								if (sanitiseOutput) {
									...
									for (String name : ao.getNames()) {
										aoNames.add(RegexUtils
												.sanitiseCommonEscapeCharactersIntoAscii(name));
									}
								}

								if (aoNames != null) {
									List<String> aoNameList = new ArrayList<String>(
											aoNames);
									row.add(aoNameList);
								}
							}
							...
							if (sanitiseOutput && rawVal instanceof String) {
								String stringVal = GenericConvert
										.toString(rawVal);
								row.add(stringVal);
							} else {
								row.add(rawVal);
	...
	public static RESTFunction meta_GET = (req, res) -> {
		return prepareAuthenticatedREST(
				...
				(reqObj, resMap, basePageObj, accountTableObj, currentUser,
						groupObj, accObj_b) -> {
					Map<String, Object> metaMap = mtApi.meta_GET
							.apply(req, res);
					...
					boolean sanitiseOutput = req.getBoolean("sanitiseOutput",
							true);
					Map<String, Object> commonInfoMap = extractCommonInfoFromAccountObject(
							account, sanitiseOutput);
		...
		return fetchGroupObject_fromFirstWildcard_orCurrentUser(
				...
				(reqObj, resMap, basePageObj, accountTableObj, currentUser,
					...
					try {
						...
						AccountObject[] members = groupObj
								.getMembersAccountObject();

						int listCounter = 0;
						for (AccountObject accObj : members) {
							MetaObject groupMemberMeta = groupObj
									.getMember(accObj);
								...
								if (header.equalsIgnoreCase("_oid")) {
									...
									if (sanitiseOutput) {
										...
										for (String name : accObj.getNames()) {
											names.add(RegexUtils
													.sanitiseCommonEscapeCharactersIntoAscii(name));
										}
									}
									retList.get(listCounter).add(names); // sanitise
								} else if (header.equalsIgnoreCase("role")) {
									String role = groupObj
											.getMemberRole(accObj); // sanitise
									if (sanitiseOutput) {
										role = RegexUtils
												.sanitiseCommonEscapeCharactersIntoAscii(role);
									}
									retList.get(listCounter).add(role);
								} else if (header.toLowerCase().startsWith(
										"account_")) { // might need to
														// sanitise....but will
														// come back to this
														// later
									String headerSuffix = header
											.substring("account_".length());
				...
				(reqObj, resMap, basePageObj, accountTableObj, currentUser,
					...
					try {
						...
						if (delMemberRaw != null) {
							delMember = ConvertJSON
									.toList((String) delMemberRaw);
						...
						if (delMember != null) {
							for (Object strRaw : delMember) {
								...
								if (!str.isEmpty()) {
									AccountObject memberToDelete = accountTableObj
											.getFromID(str);
						...
						if (setMemberMapRaw != null) {
							setMemberMap = ConvertJSON
									.toMap((String) setMemberMapRaw);

							successfulAdds = new ArrayList<String>();
						}

						if (setMemberMap != null) {
							for (Entry<String, Object> setMember : setMemberMap
									.entrySet()) {
								AccountObject newMember = accountTableObj
										.getFromID(setMember.getKey());
								if (newMember != null) {
									// groupObj.addMember(newMember,
									// (String)setMember.getValue()).saveAll();
									groupObj.setMember(newMember,
											(String) setMember.getValue())
											.saveDelta();
	...
	public static RESTFunction members_meta_GET = (req, res) -> {
		// Only runs function if logged in, and valid group object
		return fetchGroupObject_fromFirstWildcard_orCurrentUser(req,
				res,
				false,
				(reqObj, resMap, basePageObj, accountTableObj, currentUser,
						groupObj, accObj_b) -> {
				...
				for (String key : userInfoKeys) {
					...
					if (rawVal instanceof String) {
						groupUserInfo.put(
								key,
								RegexUtils
										.sanitiseCommonEscapeCharactersIntoAscii(GenericConvert
												.toString(rawVal)));
					} else {
	...
	public static RESTFunction members_meta_POST = (req, res) -> {
		// Only runs function if logged in, and valid group object
		return fetchGroupObject_fromFirstWildcard_orCurrentUser(req, res,
				false, (reqObj, resMap, basePageObj, accountTableObj,
	...
	public static RESTFunction new_account_POST = (req, res) -> {
		...
		return prepareAuthenticatedREST(
				...
				(reqObj, resMap, basePageObj, accountTableObj, currentUser,
					...
					AccountObject newAccount = accountTableObj
							.newObject(userName);
					if (newAccount != null) {
						if (isGroup) {
						...
						AccountObject existingAccount = accountTableObj
								.getFromName(userName);
						res.put("accountID", existingAccount._oid());
						res.put("error",
								"Object already exists in account Table");
					}
		...
		return prepareAuthenticatedREST(
				...
				(reqObj, resMap, basePageObj, accountTableObj, currentUser,
					...
					if ((accountID == null || accountID.isEmpty())
						...
						res.put("error",
								"Both accountID and accountName were not supplied");
						return resMap;
					}
	...
	public static RESTBuilder setupRESTBuilder(RESTBuilder rb, AccountTable at,
		...
		rb.getNamespace(setPrefix + "login")
				.put(HttpRequestType.GET, login_GET);
		rb.getNamespace(setPrefix + "login").put(HttpRequestType.POST,
				login_POST);
		rb.getNamespace(setPrefix + "logout").put(HttpRequestType.GET,
				logout_GET);
		rb.getNamespace(setPrefix + "password").put(HttpRequestType.POST,
				password_POST);

		rb.getNamespace(setPrefix + "info/name").put(HttpRequestType.GET,
				infoByName_GET);
		rb.getNamespace(setPrefix + "info/name/*").put(HttpRequestType.GET,
				infoByName_GET);

		rb.getNamespace(setPrefix + "info/id").put(HttpRequestType.GET,
				infoByID_GET);
		rb.getNamespace(setPrefix + "info/id/*").put(HttpRequestType.GET,
				infoByID_GET);

		rb.getNamespace(setPrefix + "members/list/*").put(HttpRequestType.GET,
				members_list_GET);
		rb.getNamespace(setPrefix + "members/list").put(HttpRequestType.GET,
				members_list_GET);

		rb.getNamespace(setPrefix + "members/list/*").put(HttpRequestType.POST,
				members_list_POST);
		rb.getNamespace(setPrefix + "members/list").put(HttpRequestType.POST,
				members_list_POST);

		rb.getNamespace(setPrefix + "members/meta/*").put(HttpRequestType.GET,
				members_meta_GET);
		rb.getNamespace(setPrefix + "members/meta/*/*").put(
				HttpRequestType.GET, members_meta_GET);

		rb.getNamespace(setPrefix + "members/meta/*").put(HttpRequestType.POST,
				members_meta_POST);
		rb.getNamespace(setPrefix + "members/meta/*/*").put(
				HttpRequestType.POST, members_meta_POST);

		rb.getNamespace(setPrefix + "new").put(HttpRequestType.POST,
				new_account_POST);
		rb.getNamespace(setPrefix + "delete").put(HttpRequestType.POST,
				delete_account_POST);

		// MetaTableApiBuilder Fall through
		rb.getNamespace(setPrefix + "meta/list").put(HttpRequestType.GET,
				list_GET_and_POST);
		rb.getNamespace(setPrefix + "meta/list").put(HttpRequestType.POST,
				list_GET_and_POST);

		rb.getNamespace(setPrefix + "meta/*")
				.put(HttpRequestType.GET, meta_GET);
		rb.getNamespace(setPrefix + "meta/*").put(HttpRequestType.POST,
				meta_POST);
		rb.getNamespace(setPrefix + "meta").put(HttpRequestType.GET, meta_GET);
		rb.getNamespace(setPrefix + "meta")
				.put(HttpRequestType.POST, meta_POST);

		rb.getNamespace(setPrefix + "delete").put(HttpRequestType.DELETE,
				meta_DELETE);
		rb.getNamespace(setPrefix + "delete/*").put(HttpRequestType.DELETE,
				meta_DELETE);
	...
	public void doSetup() throws Exception {
		super.doSetup();
		setupRESTBuilder(this.restBuilder(), this.accountAuthTable(),
				_apiSetPrefix);
	}
