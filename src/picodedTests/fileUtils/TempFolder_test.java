...
public class TempFolder_test {
	...
	public static void setUp() {
		...
		try {
			parentFile = new File(TempFolder.getSystemTempRootPath() + "/"
					+ _tempFolderName);
	...
	public static void checkWrittenData() {
		...
		try {
			...
			File writeStringFile = new File(parentFile.getPath() + "/"
					+ writeStringFileName);
			String val = FileUtils.readFileToString(writeStringFile);
			assertEquals("hello there", val);

			File writeByteArrayFile = new File(parentFile.getPath() + "/"
					+ writeByteArrayFileName);
	...
	public void writeStringToChildFile() {
		...
		try {
			...
			assertTrue(_tempFolder.writeToChildFile(childFileName,
					"hello there"));
		} catch (Exception e) {
	...
	public void writeByteArrayToChildFile() {
		...
		try {
			...
			assertTrue(_tempFolder.writeToChildFile(childFileName, new byte[] {
					1, 2, 3, 4, 5 }));
		} catch (Exception e) {
