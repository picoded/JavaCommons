...
public class FormInputTemplates {
	...
	private static String thousandsSeparator(String value, boolean addDecimal) throws RuntimeException {
		String ret = "";
	...
	protected static StringBuilder dropdownWithOthers(FormNode node, boolean displayMode) {
		if (!displayMode) {
			...
			String nodeName = RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(node.getString(JsonKeys.FIELD))
				.toLowerCase();
			...
			String valLowercased = RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(val).toLowerCase();
			...
			if (nodeDefaultVal != null) {
				if (nodeDefaultVal instanceof String) {
					...
					for (String str : (List<String>) nodeDefaultVal) {
						String sanitisedSelection = RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(str);
						sanitisedSelection = RegexUtil.removeAllWhiteSpace(sanitisedSelection);
						checkboxSelections.add(sanitisedSelection.toLowerCase());
					}
				}
	...
	private static List<String> dropdownKeyList(Object dropDownObject) {
		...
		if (dropDownObject instanceof List) {
			for (int a = 0; a < nameList.size(); ++a) {
				keyList.add(RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(nameList.get(a)).toLowerCase());
			}
		} else if (dropDownObject instanceof Map) {
			Map<Object, Object> dropDownMap = (Map<Object, Object>) dropDownObject;
			for (Object keyObj : dropDownMap.keySet()) {
	...
	private static Map<String, String> optionsKeyNamePair(Object optionsObject) {
		...
		if (optionsObject instanceof List) {
			...
			for (String name : nameList) {
				String sanitisedName = RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(name).toLowerCase();
			...
			for (String key : optionsMap.keySet()) {
				String sanitisedKey = RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(key).toLowerCase();
		...
		for (Object childRaw : children) {
			if (childRaw instanceof Map) {
				...
				if (childMap.containsKey("field")) {
					ret.add(RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators((String) childMap.get("field"))
						.toLowerCase());
				}
			}
		}
	...
	protected static String getDropDownOthersJavascriptFunction(FormNode node) {
		String dropDownField = RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(node.getString(JsonKeys.FIELD))
			.toLowerCase();
		String inputField = node.getString(JsonKeys.DROPDOWN_WITHOTHERS_TEXTFIELD);
		String othersOptionToShowTextField = RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(
