...
echo "Running single test: picodedTests."$1"_test"#$2
echo "---------------------------------------------------------------------------------"

#-Djava.library.path="./build-tools/junit/*.jar:./bin/build/picodedJavaCommons-libsOnly.jar"
java -cp "./build-tools/junit/*:./bin/build/picodedJavaCommons-libsOnly.jar:./bin/classes" picodedTests.SingleJUnitTestRunner picodedTests."$1"_test#"$2"
