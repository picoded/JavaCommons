...
public class DeleteFilesByAge {
	...
	static public void olderThenUnixTimestamp(File fileObj,
			long olderThenUnixTimestamp) {
		if (fileObj.isDirectory()) {
			if (fileObj.listFiles().length == 0
					&& fileObj.lastModified() <= olderThenUnixTimestamp) {
				fileObj.delete();
			} else {
				...
				&& (fileObj.lastModified() / 1000L) <= olderThenUnixTimestamp) { // actual
																					// deleting?
			fileObj.delete();
		}
	}
	...
	// deleted
	static public void olderThenGivenAgeInSeconds(File fileObj,
			long ageInSeconds) {
	...
	// / @param ageInSeconds The age in seconds, that the file must be to be
	// deleted
	static public void olderThenGivenAgeInSeconds(String filePath,
			long ageInSeconds) {
		DeleteFilesByAge.olderThenGivenAgeInSeconds(new File(filePath),
				ageInSeconds);
	}
}