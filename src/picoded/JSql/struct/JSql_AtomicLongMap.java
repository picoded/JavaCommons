...
public class JSql_AtomicLongMap extends JStruct_AtomicLongMap {
	...
	public boolean weakCompareAndSet(String key, Long expect, Long update) {
		...
		try {
			// Search for the key
			JSqlResult r = sqlObj.selectQuerySet(sqlTableName, "*", "kID=? AND kVl=?", new Object[] { key, expect }).query();
			...
			if(oldVal == null || r.rowCount() == 0){
				...
				if(oldVal.equals(expect)){
					return sqlObj.execute("UPDATE " + sqlTableName + " SET kVl= ? WHERE kID = ? AND kVl = ?", update, key, expect);
				}else{
					return false;
				}
			}
