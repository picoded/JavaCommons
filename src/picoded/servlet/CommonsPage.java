...
public class CommonsPage extends BasePage {
	...
	Runnable backgroundThreadHandler = () -> {
		// The config to use
		GenericConvertMap<String,Object> bgConfig = DConfig().getGenericConvertStringMap("sys.background","{}");
		...
		while( isBackgroundThreadAlive() ) {
			...
			try {
				...
				log().warning("          Note that the 'backgroundProcess' should be designed to never throw an exception,");
				log().warning("          As it will simply be ignored and diverted into the logs (with this message)");
			...
			try {
				...
				if( isBackgroundThreadAlive() ) {
					log().info("backgroundThreadHandler - caught InterruptedException (possible termination event)");
				} else {
					log().warning("backgroundThreadHandler - caught Unexpected InterruptedException (outside termination event)");
				}
			}
	...
	void backgroundThreadHandler_stop() {
		...
		if( backgroundThread != null ) {
			...
			try {
				backgroundThread.join( DConfig().getLong("sys.background.contextDestroyJoinTimeout", 10000) );
			} catch(InterruptedException e) {
				log().warning("backgroundThreadHandler - Unexpected InterruptedException on Thread.join : "+e.getMessage());
				log().warning(picoded.core.exception.ExceptionUtils.getStackTrace(e));
			}
