...
public class TempFolder {
	private interface fileWriterInterface extends
			BiFunction<File, Object, Boolean> {
...
//	private final static String fileExtension = ".tmp";
	...
	public boolean writeToChildFile(String childFileName, Object data) {
		...
		File childFile = new File(_tempFolder.getPath() + File.separator
				+ childFileName);
	...
	public File getTempFolder() throws IOException {
		if (_tempFolder == null) {
			...
			if (!createdSuccessfully) {
				throw new RuntimeException(
						"getTempFolder() -> Failed to create temp folder");
			}
		}
		return _tempFolder;
	}
	...
	public static String getSystemTempRootPath() {
		try {
			...
			throw new RuntimeException(
					"getTempDir() -> Unable to access tmp dir property: "
							+ sex.getMessage());
		} catch (Exception ex) {
			throw new RuntimeException(
					"getTempDir() -> Error while trying to get tmp dir property: "
							+ ex.getMessage());
		}
	}
