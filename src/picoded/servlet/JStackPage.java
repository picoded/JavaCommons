...
public class JStackPage extends CorePage {
	...
	public String getPageTemplatePath() {
		return (_pageTemplatePath != null) ? _pageTemplatePath : (_pageTemplatePath = getWebInfPath()
			+ "page/");
	}
	...
	public String getJsmlTemplatePath() {
		return (_jsmlTemplatePath != null) ? _jsmlTemplatePath : (_jsmlTemplatePath = getWebInfPath()
			+ "jsml/");
	}
	...
	protected JStackLayer[] loadConfiguredJStackLayers() {
		...
		if (stackOptions == null || stackOptions.size() <= 0) {
			System.out
				.println("---JStackPage -> loadConfiguredJStackLayers -> stackOptions is null or empty---");
		}
		...
		if (profileType.equalsIgnoreCase("JSql")) {
			...
			throw new RuntimeException("Unsupported sys.dataStack.selected.profile.type: "
				+ profileType);
		}
	}
	...
	public void initializeContext() throws Exception {
		...
		boolean skipSystemSetup = JConfig().getBoolean("sys.JStack.skipSystemSetup", false);
		if (!skipSystemSetup) {
			JStack().systemSetup();
		} else {
