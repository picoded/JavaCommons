...
public class FormInputTemplates {
	...
	private static String thousandsSeparator(String value, boolean addDecimal)
		...
		if (value != null && !value.trim().isEmpty()) {
			if (value.equalsIgnoreCase("0") || value.equalsIgnoreCase("0.00")) {
				...
				tempValue = RegexUtil.removeAllNonNumeric(tempValue);
				
				BigDecimal bigD = new BigDecimal(tempValue);
				// no need to round it, DecimalFormat will round it
				// bigD = bigD.setScale(2, RoundingMode.HALF_UP);
				
				// This method will ignore the settings af addDecimal, it will follow the value from UI
				// this is kind of defeat the purpose of addDecimal, anyway when the value is 0 it force the 0 have decimal
				// this fixes is to set the MaximumFractionDigits to match the number of decimal from tempValue
				// For matching to original design, the MaximumFractionDigits is limit to 2 
				// scale() is get the decimal digit from tempValue
				// Case 1
				// when tempValue = "123.00" 
				// new BigDecimal(tempValue) = 123
				// scale() = 2
				// Case 2
				// when tempValue = "123" 
				// new BigDecimal(tempValue) = 123
	...
	protected static FormInputInterface select = (node) -> {
		...
		StringBuilder[] sbArr = node.defaultHtmlInput(HtmlTag.SELECT, "pfi_select pfi_input",
			paramsMap);
	...
	protected static FormInputInterface input_text = (node) -> {
		...
		StringBuilder[] sbArr = node.defaultHtmlInput(HtmlTag.INPUT, "pfi_inputText pfi_input",
			paramMap);
	...
	protected static FormInputInterface input_number = (node) -> {
		...
		StringBuilder[] sbArr = node.defaultHtmlInput(HtmlTag.INPUT, "pfi_inputNumber pfi_input",
			paramMap);
		return sbArr[0].append(sbArr[1]);
	};
	...
	protected static StringBuilder input_textarea(FormNode node, boolean displayMode) {
		...
		if (!displayMode) {
			...
			boolean separateParagraph = node.getBoolean("separateParagraph", true);
			int maxLines = node.getInt("maxLines", -1);
			int curLine = 1;
			String[] fieldValParaSplit = fieldValue.split("\n");
			for (String para : fieldValParaSplit) {
				if (maxLines > -1 && curLine > maxLines) {
					break;
				}
				...
				newPara = paraNode.defaultHtmlInput(inputType,
					"pfi_inputTextBox pfi_input pfi_display", null);
				sbArr[0].append(newPara[0]);
				sbArr[0].append(para);
				sbArr[0].append(newPara[1]);
				
				if (separateParagraph) {
	...
	protected static StringBuilder dropdownWithOthers(FormNode node, boolean displayMode) {
		if (!displayMode) {
			...
			String nodeName = RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(
				node.getString(JsonKeys.FIELD)).toLowerCase();
			...
			StringBuilder ret = new StringBuilder(getDropDownOthersJavascriptFunction(node)
				+ sbArr[0].toString());
			...
			StringBuilder[] inputTextArr = node.defaultHtmlInput(HtmlTag.INPUT, "pf_inputText",
				inputParamMap);
			...
			String valLowercased = RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(val)
				.toLowerCase();
	...
	protected static StringBuilder createCheckbox(FormNode node, boolean displayMode, String pfiClass) {
				...
				if (nodeDefaultVal instanceof String) {
					...
					if (nodeValString.contains("[")) {
						...
						for (Object obj : nodeValMap) {
							String sanitisedSelection = RegexUtil
								.removeAllNonAlphaNumeric_allowCommonSeparators((String) obj);
							sanitisedSelection = RegexUtil.removeAllWhiteSpace(sanitisedSelection);
					...
					for (String str : (List<String>) nodeDefaultVal) {
						String sanitisedSelection = RegexUtil
							.removeAllNonAlphaNumeric_allowCommonSeparators(str);
						sanitisedSelection = RegexUtil.removeAllWhiteSpace(sanitisedSelection);
						checkboxSelections.add(sanitisedSelection.toLowerCase());
					}
				}
		...
		for (String key : keyNamePair.keySet()) {
			if (!displayMode) {
				CaseInsensitiveHashMap<String, String> tempMap = new CaseInsensitiveHashMap<String, String>(
					paramMap);
				tempMap.put("value", key);
				String key_sanitised = RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(key);
				key_sanitised = RegexUtil.removeAllWhiteSpace(key).toLowerCase();
				...
				if (removeDefaultSaveFunction == false && realName != null && !realName.isEmpty()) {
					String onChangeFunctionString = "saveCheckboxValueToHiddenField('" + realName
						+ "_dummy', '" + realName + "')";
					tempMap.put("onchange", onChangeFunctionString);
				}
				
				StringBuilder[] sbArr = tempNode.defaultHtmlInput(HtmlTag.INPUT,
					"pfi_inputCheckbox pfi_input", tempMap);
				...
				StringBuilder[] sbArr = node.defaultHtmlInput(HtmlTag.DIV,
					"pfi_inputCheckbox pfi_input", null);
				...
				for (String selection : checkboxSelections) {
					if (key.equalsIgnoreCase(selection)) {
						sbArr[0]
							.append("<div class=\"pf_displayCheckbox pf_displayCheckbox_selected\"></div>");
						found = true;
					}
				}
				...
				if (!found) {
					sbArr[0]
						.append("<div class=\"pf_displayCheckbox pf_displayCheckbox_unselected\"></div>");
				}
				
				if (showValueAsLabelText) {
					sbArr[0].append("<div class=\"pf_displayCheckbox_text\">" + keyNamePair.get(key)
						+ "</div>");
				}
		...
		if (checkboxSelections != null && checkboxSelections.size() > 0) {
			hiddenInputTag = "<input type=\"text\" class=\"pfi_input\" style=\"display:none\" name=\""
				+ realName + "\" value='" + jsonValue + "'></input>";
		} else {
			hiddenInputTag = "<input type=\"text\" class=\"pfi_input\" style=\"display:none\" name=\""
				+ realName + "\"></input>";
		}
	...
	protected static StringBuilder tableWrapper(FormNode node, boolean displayMode) {
		...
		//<table> tags
		StringBuilder[] wrapperArr = node.defaultHtmlWrapper(HtmlTag.TABLE, node.prefix_standard()
			+ "div", null);
		...
		for (Map<String, Object> childValues : clientsValues) {
			...
			for (Map<String, String> childMap : childData) {
				...
				FormWrapperInterface func = node._formGenerator.wrapperInterface(displayMode,
					childNodeType);
	...
	protected static StringBuilder signature(FormNode node, boolean displayMode) {
		...
		if (!displayMode) {
			ret.append("<h3 class=\"pfi_input pfi_inputSigDate\" name=\"" + dateInputId + "\" id=\""
				+ dateInputId + "\" value=\"" + dateValue + "\">" + textString + "</h3>");
		} else {
			if (!dateValue.isEmpty()) {
				ret.append("<h3 class=\"pfi_input pfi_inputSigDate\" name=\"" + dateInputId
					+ "\" id=\"" + dateInputId + "\">" + textString + "</h3>");
			}
		}
	...
	protected static StringBuilder datePicker(FormNode node, boolean displayMode) {
		...
		if (!displayMode) {
			...
			if (node.containsKey("max")) {
				String maxDate = DateConv.changeISODateFormat(node.getString("max"),
					ISODateFormat.YYYYMMDD, ISODateFormat.YYYYMMDD, "-");
			...
			if (node.containsKey("min")) {
				String minDate = DateConv.changeISODateFormat(node.getString("min"),
					ISODateFormat.YYYYMMDD, ISODateFormat.YYYYMMDD, "-");
			...
			StringBuilder[] sbArr = node.defaultHtmlInput(HtmlTag.INPUT, "pfi_inputDate pfi_input",
				paramMap);
			ret.append(sbArr[0]);
			ret.append(sbArr[1]);
			if (!displayMode) {
			...
			StringBuilder[] sbArr = node
				.defaultHtmlInput(HtmlTag.DIV, "pfi_inputDate pfi_input", null);
	...
	private static List<String> dropdownKeyList(Object dropDownObject) {
		...
		if (dropDownObject instanceof List) {
			for (int a = 0; a < nameList.size(); ++a) {
				keyList.add(RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(nameList.get(a))
					.toLowerCase());
			}
		} else if (dropDownObject instanceof Map) {
			Map<Object, Object> dropDownMap = (Map<Object, Object>) dropDownObject;
			for (Object keyObj : dropDownMap.keySet()) {
				String key = RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(
	...
	private static Map<String, String> optionsKeyNamePair(Object optionsObject) {
		...
		if (optionsObject instanceof List) {
			...
			for (String name : nameList) {
				String sanitisedName = RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(name)
					.toLowerCase();
				sanitisedName = RegexUtil.removeAllWhiteSpace(sanitisedName);
			...
			for (String key : optionsMap.keySet()) {
				String sanitisedKey = RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(key)
					.toLowerCase();
				sanitisedKey = RegexUtil.removeAllWhiteSpace(sanitisedKey);
	...
	private static void createDropdownHTMLString(StringBuilder sb, List<String> keyList,
		List<String> nameList, String selectedKey) {
		...
		for (Object childRaw : children) {
			if (childRaw instanceof Map) {
				...
				if (childMap.containsKey("field")) {
					ret.add(RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(
						(String) childMap.get("field")).toLowerCase());
				}
			}
		}
		...
		if (node.containsKey("headers")) {
			...
			if (!(tableHeadersRaw instanceof List)) {
				throw new IllegalArgumentException(
					"'tableHeader' parameter found in defination was not a List: " + tableHeadersRaw);
			}
	...
	protected static String getDropDownOthersJavascriptFunction(FormNode node) {
		String dropDownField = RegexUtil.removeAllNonAlphaNumeric_allowCommonSeparators(
			node.getString(JsonKeys.FIELD)).toLowerCase();
		String inputField = node.getString(JsonKeys.DROPDOWN_WITHOTHERS_TEXTFIELD);
		String othersOptionToShowTextField = RegexUtil
			.removeAllNonAlphaNumeric_allowCommonSeparators(node.getString(JsonKeys.OTHERS_OPTION))
			.toLowerCase();
		String funcName = node.getString(JsonKeys.FUNCTION_NAME, "OnChangeDefaultFuncName");
