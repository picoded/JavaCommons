...
public class ServletLogging {
   ...
   private void tableSetup(JSql jSqlObj) throws JSqlException {
      ...
      if (jSqlObj.sqlType == JSqlType.sqlite) {
         jSqlObj.execute("CREATE TABLE IF NOT EXISTS `config` ( "
                         + "key VARCHAR(" + objColumnLength + "), sVal TEXT, PRIMARY KEY (key) );");
      }
      ...
      // / excStrHash table
      jSqlObj.execute("CREATE TABLE IF NOT EXISTS `excStrHash` ( "
                      + "hash VARCHAR(" + objColumnLength + "), sVal TEXT, PRIMARY KEY (hash) );");

      // / exception table
      jSqlObj.execute("CREATE TABLE IF NOT EXISTS `exception` ( "
                      + "expHash VARCHAR(" + objColumnLength + "), "
                      + "reqsID VARCHAR(" + objColumnLength + "), "
                      + "creTime BIGINT, instID VARCHAR(60), "
                      + "excRoot VARCHAR(" + valColumnLength + "), "
                      + "excTrace VARCHAR(" + valColumnLength + "), "
                      + "excR01-XX VARCHAR(" + valColumnLength + "), "
                      + "excT01-XX VARCHAR(" + valColumnLength + "), "
                      + "excMid VARCHAR(" + valColumnLength + "),"
                      + "stkRoot VARCHAR(" + valColumnLength + "), "
                      + "stkTrace VARCHAR(" + valColumnLength + "), "
                      + "stkR01-XX VARCHAR(" + valColumnLength + "), "
                      + "stkT01-XX VARCHAR(" + valColumnLength + "), "
                      + "stkMid VARCHAR(" + valColumnLength + "),"
                      + "PRIMARY KEY (expHash) );");

      // / logTable table
      jSqlObj.execute("CREATE TABLE IF NOT EXISTS `logTable` ( "
                      + "instID VARCHAR(" + objColumnLength + "), "
                      + "reqsID VARCHAR(" + objColumnLength + "), "
                      + "creTime BIGINT, "
                      ...
                      + "l01-XX VARCHAR(" + valColumnLength + "), "
                      + "s01-XX VARCHAR(" + valColumnLength + "), "
                      + "lXX-YY VARCHAR(" + valColumnLength + "), "
   ...
   public String addFormat(String formatName, String formatString) throws Exception {
      ...
      jSqlObj.execute( "INSERT INTO `logFormat` (hash, format, name) VALUES (?, ?, ?)", fmtHash, formatString, formatName );
      return fmtHash;
   }
   ...
   private String generateHash() throws Exception {
      ...
      byte[] messageDigest = md.digest(UUID.randomUUID().toString()
                                       .getBytes());
      String s = new Base58().encode(messageDigest);
      if (s.length() < 22) {
         throw new RuntimeException(
            "GUID generation exception, invalid length of "
            + s.length() + " (" + s + ")");
      }
