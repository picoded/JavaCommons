...
public class AccountTableApi extends CommonApiModule {
	...
	protected ApiFunction get_member_list_info = (req, res) -> {
		...
		String[] headers = req.getStringArray(REQ_HEADERS, "['" + PROPERTIES_OID + "', '"
			+ PROPERTIES_ROLE + "']");
	...
	protected ApiFunction reset_password = (req, res) -> {
		...
		String[] paramsToCheck = new String[] { REQ_OLD_PASSWORD, REQ_NEW_PASSWORD,
			REQ_REPEAT_PASSWORD };
		res = check_parameters(paramsToCheck, req, res);
		if (res.get(RES_ERROR) != null)
			return res;
	...
	protected ApiFunction update_current_user_info = (req, res) -> {
		...
		if (res.get(RES_ERROR) != null)
			return res;
		Object metaObjRaw = req.getStringMap(REQ_META);
		String updateMode = req.getString(REQ_UPDATE_MODE, "delta");
	...
	protected ApiFunction account_info_by_Name = (req, res) -> {
		String userName = req.getString(REQ_USERNAME, "");
		AccountObject ao = (!userName.isEmpty()) ? table.getFromLoginID(userName) : table
			.getRequestUser(req.getHttpServletRequest(), null);
		if (ao == null) {
			res.put(RES_ERROR, ERROR_NO_USER);
			return res;
