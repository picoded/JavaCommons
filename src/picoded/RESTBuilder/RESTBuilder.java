...
public class RESTBuilder {
	...
	protected static String apiNodeBuilderJS(String builderName, String baseURL ) {
		...
		return ""+ //
			xmlHttpJS(xHttpCall)+//
			//
			"var apiHost = '"+baseURL+"'; "+ //the base url hostname
			//
			"function filterNamespace(n) { "+ //
				//
				"if(n != null && n.length > 0) { "+//Check if there is content
					"n=n.replace(/\\/\\*/g, '/').replace(/\\/\\//g, '/').replace(/\\/\\//g, '/'); "+//
					//
					"if(n.indexOf('/') !== 0) { "+//Enforces starting / as it is needed
						"n='/'+n; "+//
					"} "+//
					//
					"if(n[n.length-1] == '/') { "+//
			...
			"} "+ //
			//
			"function "+builderName+"(n) { "+//
				"n = filterNamespace(n); "+//
				"var api = function(namespace) { "+//
					"return "+builderName+"(n+'/'+namespace); "+//
				"}; "+//
				// The url that this node calls
				"var tar = apiHost+n; "+//
				// Setsup the various GET,PUT,POST,DELETE
				"api.GET=function(p,c){ return "+xHttpCall+"(tar, 'GET', p, c); }; "+
				"api.PUT=function(p,c){ return "+xHttpCall+"(tar, 'PUT', p, c); }; "+
				"api.POST=function(p,c){ return "+xHttpCall+"(tar, 'POST', p, c); }; "+
				"api.DELETE=function(p,c){ return "+xHttpCall+"(tar, 'DELETE', p, c); }; "+
				//
				"return api; "+//
			"} "; //
	}
